...
int main()
{
	int i, j, contL1=0, contL2=0, contL3=0, contL4=0, soma=0, qtdsoma=0, x[4][3];
	float media;
	...
	for (i=0 ; i<4 ; i++)
	{
		for (j=0 ; j<3 ; j++)
		{
			...
			if(10 <= x[i][j] && x[i][j] <= 20 && i==0){
				contL1+=1;
			} else{
				if(10 <= x[i][j] && x[i][j] <= 20 && i==1){
					contL2+=1;
				} else{
					if(10 <= x[i][j] && x[i][j] <= 20 && i==2){
					contL3+=1;
					}else{
						if(10 <= x[i][j] && x[i][j] <= 20 && i==3){
						contL4+=1;
						}
					}
				} 
	...
	printf ("%d numeros estao entre 10 e 20 na quarta linha.\n", contL4);
