...
def medir_tempo():
    ...
    resultados = {
        "Busca2": {
            "Binária": {"entradas": entradas_normais, "tempos": []}
        }
    }
    ...
    for n in entradas_normais:
        ...
        t = timeit.timeit(lambda: busca_binaria(lista_ordenada, alvo), number=num_repeticoes)
        resultados["Busca2"]["Binária"]["tempos"].append(t)
...
def gerar_graficos(resultados):
    ...
    for titulo, dados_algoritmo in resultados.items():
        ...
        plt.title(f'Desempenho: Busca Binária')
